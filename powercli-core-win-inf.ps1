...
$DomainControllerVMName = "dc01.core.$fqdn"
$FSVMName = "fs01.core.$fqdn"
$TargetCluster = Get-Cluster -Name "Omega-Core"
$SourceVMTemplate = Get-Template -Name "omega-core.windows.server.2022.core"
$SourceCustomSpec = Get-OSCustomizationSpec -Name "Omega-Core-Win"
...
$DCNetworkSettings = 'netsh interface ip set address "Ethernet0" static 10.1.5.100 255.255.255.0 10.1.5.2'

# FS VM IPs Below
# NOTE: Insert IP info in IP SubnetMask Gateway Order
##CM Add code for P or V networks for naming schemes
$FSNetworkSettings = 'netsh interface ip set address "Ethernet0" static 10.1.11.105 255.255.255.0 10.1.5.2'
# NOTE: DNS Server IP below should be the same IP as given to the domain controller in the $DCNetworkSettings Variable
$FSDNSSettings = 'netsh interface ip set dnsservers name="Ethernet0" static 10.1.5.100 primary'
...
$DCLocalUser = "$DomainControllerVMName\admin.omega"
$DCLocalPWord = ConvertTo-SecureString -String "OMEGApass12!@" -AsPlainText -Force
...
$FSLocalUser = "$FSVMName\admin.omega"
$FSLocalPWord = ConvertTo-SecureString -String "OMEGApass12!@" -AsPlainText -Force
...
$DomainUser = "omega.engineering\admin.omega"
$DomainPWord = ConvertTo-SecureString -String "OMEGApass12!@" -AsPlainText -Force
...
$JoinNewDomain = '$DomainUser = "omega.engineering\admin.omega";
                  $DomainPWord = ConvertTo-SecureString -String "OMEGApass12!@" -AsPlainText -Force;
                  $DomainCredential = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList $DomainUser, $DomainPWord;
                  Add-Computer -DomainName omega.engineering -Credential $DomainCredential;
...
$ConfigureNewDomain = 'Write-Verbose -Message "Configuring Active Directory" -Verbose;
                       ...
                       $DomainName = "omega.engineering";
                       $DSRMPWord = ConvertTo-SecureString -String "OMEGApass12!@" -AsPlainText -Force;
...
$NewAdminUser = '$AdminUserPWord = ConvertTo-SecureString -String "OMEGApass12!@" -AsPlainText -Force;
                 New-ADUser -Name "oe.admin" -AccountPassword $AdminUserPWord;
                 Add-ADGroupMember -Identity "Domain Admins" -Members "oe.admin";
                 Enable-ADAccount -Identity "oe.admin"'
